...
@app.route("/cancel-email", methods=["POST"])
def cancel_email():
    try:
        ...
        result = cancel_scheduled_email(job_id, reason)
...
def upcoming_deliveries():
    """Return a list of upcoming scheduled deliveries.
    Uses in-memory email_status_db and sorts by scheduled_time.
    """
    from scheduler import email_status_db
    items = []
    for job_id, rec in email_status_db.items():
        status = rec.get("status")
        scheduled_time = rec.get("scheduled_time")
        if status in ("scheduled", "pending") and scheduled_time:
            items.append({
                "job_id": job_id,
                "recipient": rec.get("recipient") or rec.get("to_email"),
